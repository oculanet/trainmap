...
def dashboard():
    ...
    stats = {
        ...
        'mainline_trains': c.execute("SELECT COUNT(*) FROM trains t JOIN tracks tr ON t.track_id = tr.track_id WHERE tr.track_type = 'MAINLINE'").fetchone()[0]
    }
...
def update_train(train_number):
    ...
    if request.method == 'POST':
        ...
        current_track = c.fetchone()['track_id']
        
        # Get track name for movement log
        ...
        try:
            ...
            c.execute("""
                UPDATE trains 
                SET status = ?, maintenance_code = ?, track_id = ?, last_movement = datetime('now') 
                WHERE train_number = ?
            """, (new_status, maintenance_code, new_track_id, train_number))
            ...
            c.execute("""
                ...
                VALUES (?, ?, ?, ?, ?)
            """, (train_number, from_location, to_location, session['username'], comments))
    ...
    c.execute("SELECT * FROM tracks ORDER BY depot_code, track_type, track_id")
    tracks = c.fetchall()
    
    # Get maintenance types
    c.execute("SELECT * FROM maintenance_types")
